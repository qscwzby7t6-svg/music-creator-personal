...
class DeepSeekService {
  ...
  constructor() {
    this.client = new OpenAI({
      apiKey: process.env.DEEPSEEK_API_KEY || 'sk-9c295285116547729e8deee1255030aa',
      baseURL: 'https://api.deepseek.com',
    })
  }

  async analyzeStyle(audioData?: Buffer): Promise<any> {
    try {
...
请用JSON格式返回。`,
  ...
  async analyzeMelody(audioData?: Buffer): Promise<any> {
    try {
      const response = await this.client.chat.completions.create({
        model: 'deepseek-chat-v4-pro',
        messages: [
...
请用JSON格式返回数值化的分析结果。`,
  ...
  async generateLyrics(stylePrompt: string, melodyPrompt: string, theme?: string): Promise<any> {
    try {
      const response = await this.client.chat.completions.create({
        model: 'deepseek-chat-v4-pro',
        messages: [
